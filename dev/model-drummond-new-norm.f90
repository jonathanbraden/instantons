...
#define POTENTIAL(f) ( del*(cos(f)-1.) + sin(f)**2 )
#define VPRIME(f) ( -del*sin(f) + sin(2._dl*f) )
#define VDPRIME(f) ( -del*cos(f) + 2._dl*cos(2._dl*f) )
...
contains
  ...
  subroutine grid_params_(w,len,r0,w0)
    ...
    real(dl), parameter :: wscl = 5.73_dl 
  ...
  subroutine bubble_parameters_nd_(eps,dim,r0,meff)
    real(dl), intent(in) :: eps, dim
    real(dl), intent(out) :: r0, meff

    meff = sqrt(2._dl)
    r0 = dim * sqrt(2._dl)/eps
