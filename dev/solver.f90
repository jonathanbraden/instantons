...
contains
  ...
  subroutine intialise_solver(this,n)
    ...
    this%nVar = n; maxIter = 100; kick_param = 0.1_dl
...
!  end subroutine line_iteration
  ...
  subroutine line_iteration(this,f_cur)
    ...
    res = sqrt(sum(this%S*this%S))  ! Current residual
    ...
    b1 = res + kick_param ! If we're not converging, this allows us to kick ourselves
    alpha = 2._dl
    do i=1,8
       alpha = alpha/2._dl
       this%f_prev = f_cur + alpha*this%del
       this%S = source(this%f_prev) ! replace with a call to source
       this%S = this%S
    ...
    this%S = source(f_cur)          ! Store violation of the equation of motion
  end subroutine line_iteration
