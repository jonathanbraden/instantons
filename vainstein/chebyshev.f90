...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine evaluate_chebyshev(ord,x,T,nd)
    ...
    if (nd > 2) print*,"Warning, more than second order derivatives requesting for Chebyshev object.  Defaulting to up to second order derivatives"
...
!# This is dev stuff
  !>@brief
  !> Given derivatives for the forward mapping, convert them to derivatives in the inverse mapping suitable for use in transform_derivatives

  !>@brief
  !> Apply an exponential mapping to the coordinate system (useful for Vainstein)
  subroutine IR_stretch(this,m)
    type(Chebyshev), intent(inout) :: this
    real(dl), intent(in) :: m

    real(dl), allocatable :: dmap(:,:)

    print*,"IR stretching not yet fully tested"

    allocate(dmap(0:this%ord,1:2))
    this%xGrid = (exp(m*this%xGrid)-1._dl)/m
    dmap(:,1) = 1._dl / (1._dl + m*this%xGrid)
    dmap(:,2) = -m / (1._dl + m*this%xGrid)**2
    call transform_derivatives(this,dmap)
    deallocate(dmap)
  end subroutine IR_stretch
!# End of dev stuff
