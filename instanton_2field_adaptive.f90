...
!  call transform_coords(xgrid, rinit, .true.)
  ...
  call cluster_points(xgrid, 0.5, .true.)
...
!  call output_resampled()  ! output for lattice code
  call output_resampled_1d(4096*2,0.125/8.)
...
!    deltot = sum(del**2)
  ...
  subroutine get_evalues()
    ...
    do i=1,nfield
       n1 = (i-1)*nxmax + 1
       n2 = i*nxmax
       L(n1:n2,n1:n1) = -l0(XRANGE,BRANGE)
    enddo
    do i=1,nxmax
       L(i,i) = L(i,i) + vdprime(freal(i-1,:),1,1)
       L(i,i+nxmax) = vdprime(freal(i-1,:),1,2)
...
! xvals : original unmapped coords on intervals [-1:1] (in)
!       : new coordinates on intervals [0:1] appropriate for even chebys (out)
! x_to_y : if true, then original coords on [-1:1]
!          if false, coordinates of [-infinity, infinity]
...
! w controls clustering : w<1 clusters point, w>1 stretch points
