...
!!!!!!!!!!!
  ...
  real, parameter :: c1=0., c3=0.1, phi_inf=-0.1
  real :: delta
  integer, parameter :: numdelta = 1
  real, parameter, dimension(numdelta) :: deltavals = (/ 0.5 /)
  integer, parameter :: nfield=2
  real, dimension(nfield), parameter :: vac_guess = (/-1., 0./)
  real, parameter :: phiout = 0.
  real :: rinit, len
...
!!!!!!!!!!
  integer, parameter :: nmax = 127 ! order of highest interpolating polynomial
...
! Storage for various pieces of the differential operators.  These will depend on the precise equation
  ...
  real :: errtot, deltot
  real, parameter :: errtol = 5.e-12, deltol = 5.e-12
...
!Define the potential derivatives and hessian
  ...
  function vprime(phi,fld)
    ...
    if (fld == 1) then
       ...
       vprime = c1 +3.*c3*(phi(2)-phi_inf)**2 + g2*(phi(1)-1.)**2*phi(2)
    endif
  end function vprime
   ...
   real :: vdprime
    ...
    if (fld1==fld2) then
       if (fld1==1) then
          ...
          vdprime = 6.*c3*(phi(2)-phi_inf) + g2*(phi(1)-1.)**2
       endif
    else
